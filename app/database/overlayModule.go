...
func (overlay *OverlayStruct) GetModules() error {
	...
	if overlay.Modules == nil {
		overlay.Modules = make(map[int]*OverlayModule)
	}
	...
	for rows.Next() {
		...
		overlay.Modules[overlay.ID] = module
	}

	return nil
}

func (overlay *OverlayStruct) NewModule(Type OverlayModuleType, top, left, width, height int) error {
	conn, err := connectDB()
	if err != nil {
		return err
	}
